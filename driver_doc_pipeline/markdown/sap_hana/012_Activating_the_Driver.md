...
To activate the SAP HANA driver, activate the Metadirectory engine, then activate the driver by using the separate SAP HANA activation key. If you created the driver in a driver set that has not been activated, you must activate the Metadirectory engine and the driver within 90 days. Otherwise, the driver stops working.

If driver activation has expired, the following error message is displayed in the ndstrace window:

```
DirXML Log Event -------------------
Driver: \METADIRECTORY\system\DriverSet\SAP HANADriver
Channel: Subscriber
Status: Error
