...
* [Applying the Driver Patch](upgrading-the-driver.html#t43vake3x7c5)
...
2. Upgrade the installed packages.
   ...
   10. Review the upgraded package, then click OK to close the Package Management page.
       ...
       For detailed information, see "[Upgrading Installed Packages](../../../identity-manager-48/designer_admin/data/packman.html#packmanupgrade)" in the [NetIQ Designer for Identity Manager Administration Guide](../../../identity-manager-48/designer_admin/data/bookinfo.html#bookinfo).
...
6. Specify Identity Manager Oracle E-Business Suite > driver in the search box.
7. Download and unzip the contents of the patch file to a temporary location on your server.

   For example, IDM45\_OracleEBS\_4003.zip.
...
1. Update the driver files:
   ...
   * *Linux:*
     Log in to your server as root and run the following commands in a command prompt:

     rpm -Uvh <Driver Patch File Temporary Location>/linux/novell-DXMLebsCommon.rpm

     rpm -Uvh <Driver Patch File Temporary Location>/linux/novell-DXMLebsHR.rpm

     rpm -Uvh <Driver Patch File Temporary Location>/linux/novell-DXMLebsTCA.rpm

     rpm -Uvh <Driver Patch File Temporary Location>/linux/novell-DXMLebsUM.rpm

     For example, rpm -Uvh <IDM45\_OracleEBS\_4003.zip>/linux/novell-DXMLebsHR.rpm
   * *Windows:*
     Navigate to the <Extracted Driver Patch File Temporary Location>\windows folder and copy the following files to <IdentityManager installation>\NDS\lib or <IdentityManager installation>\RemoteLoader\<architecture>\lib folder.

     + EBSHRShim.jar
     + EBSUserShim.jar
     + EBSTCAShim.jar
   * *Solaris:*
     To install the driver patch on Solaris, contact NetIQ Support.
...
4. To install the driver files, enter the following command:
   ...
   For example, to install the HR driver RPM, use this command:

   ```
   rpm --dbpath $ROOTDIR/rpm -Uvh --relocate=/usr=$ROOTDIR/opt/novell/eDirectory --relocate=/etc=$ROOTDIR/etc --relocate=/opt/novell/eDirectory=$ROOTDIR/opt/novell/eDirectory --relocate=/opt/novell/dirxml=$ROOTDIR/opt/novell/dirxml --relocate=/var=$ROOTDIR/var --badreloc --nodeps --replacefiles /home/user/novell-DXMLebsHR.rpm
   ```
