...
* [Uninstalling the Security System Exit](b3xfu1c.html#b6m29nd)
...
If you were not using the TSSINSTX installation exit before installing the driver, follow [Exit Uninstallation Procedure 1](b3xfu1c.html#b6m2b8k).

If you were using the TSSINSTX installation exit before installing the driver, follow [Exit Uninstallation Procedure 2](b3xfu1c.html#b6m2blh).
...
4. Delete the users you created for the started tasks in [Preparing User IDs for the Started Tasks](b3xehpq.html#b6gwp0h).
...
7. Remove the driver files from the HFS. They were created in [Step 6](b3xehpq.html#b6a6aw2).

   ```
   rm -Rf /opt/novell
   rmdir -p /opt/novell/
   ```
8. Delete the driver samples library, load library, and REXX exec library that you created in [Step 3](b3xehpq.html#b689ggk).
9. Delete the change log data set that you created in [Allocating and Initializing the Change Log Data Set](b3xehpq.html#b689r3r).
