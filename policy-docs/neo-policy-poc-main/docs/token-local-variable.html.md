DirXMLScript DTD: token-local-variable element



# token-local-variable

**<token-local-variable>** expands to the
value of the named local variable. If this token is
evaluated in a context where a node-set result is
not expected and the variable holds a node-set,
then the string value of the node set is
returned. If the same local variable exists in
both the policy scope and the driver scope, the
variable in the policy scope takes precedence.
...
|| [**Tree**](DTD-TREE.html#token-local-variable)
