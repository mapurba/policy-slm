DirXMLScript DTD: do-add-resource element



# do-add-resource

The **<do-add-resource>** action initiates a request to the Roles Based Provisioning Module (RBPM)
to assign the Resource specified by *resource-id* to an Identity. The target Identity is specified by
...
* *success.do-add-resource* : This local variable will be available only if
  the resource is assigned successfully. And it contains the DN of the request.
* *error.do-add-resource* : This local variable will be available only if any
  type of error occurs while requesting the resource assignment. And it contains the error string.
...
|| [**Tree**](DTD-TREE.html#do-add-resource)
