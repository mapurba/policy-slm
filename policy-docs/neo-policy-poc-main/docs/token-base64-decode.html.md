DirXMLScript DTD: token-base64-decode element



# token-base64-decode

**<token-base64-decode>**decodes the
result of the enclosed tokens from base64 encoded
data to bytes and then converts the bytes into a
string using the character set specified by
*charset*.
...
> | --- | --- | --- |
> | **charset** | **CDATA**   charset used to convert the decoded bytes to a string  *if not specified, the encoding specified by the system property file.encoding is used  supports variable expansion* | #IMPLIED |
...
|| [**Tree**](DTD-TREE.html#token-base64-decode)
