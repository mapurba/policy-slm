DirXMLScript DTD: token-op-attr element



# token-op-attr

**<token-op-attr>** expands to the
attribute values in the [current
operation](policy.html#current_operation) (<add-attr>, <add-value>
or <attr>). If this token is evaluated in a
context where a node-set result is expected then
all the available <value>'s are returned as
nodes in a node-set. Otherwise the first available
...
|| [**Tree**](DTD-TREE.html#token-op-attr)
