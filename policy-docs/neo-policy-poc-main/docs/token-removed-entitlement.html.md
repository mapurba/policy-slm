DirXMLScript DTD: token-removed-entitlement element



# token-removed-entitlement

**<token-removed-entitlement>** expands to
the revoked value(s) of the named entitlement in
the [current
operation](policy.html#current_operation). If this token is evaluated in a
context where a node-set result is expected then
all the available values are returned as [<entitlement-impl>](../dirxmlentitlements/entitlement-impl.html)
...
|| [**Tree**](DTD-TREE.html#token-removed-entitlement)
