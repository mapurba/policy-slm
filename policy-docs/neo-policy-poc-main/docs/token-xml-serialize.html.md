DirXMLScript DTD: token-xml-serialize element



# token-xml-serialize

**<token-upper-case>** serializes the
node-set result enclosed tokens as XML. Depending
on the content of the node-set, the resulting
string will either be a well-formed XML document or
a well-formed parsed general entity. *Note that a
parsed general entity cannot be parsed as a
...
|| [**Tree**](DTD-TREE.html#token-xml-serialize)
