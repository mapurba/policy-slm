DirXMLScript DTD: token-dest-attr element



# token-dest-attr

**<token-dest-attr>** expands to the
attribute values of the [current
object](policy.html#current_object) in the destination datastore. If this
token is evaluated in a context where a node-set
result is expected [then all the available
<value>'s are returned as nodes in a
node-set. Otherwise the first available value is
returned as a string.](arg-node-set.html)

### [Example](arg-node-set.html)
...
> | --- | --- | --- |
> | **class-name** | **CDATA**   class name of target object  *supports variable expansion*   class name of object to read  *May be required (for schema mapping purposes) if object is other than current object..*  *supports variable expansion* | #IMPLIED |
...
|| [**Tree**](DTD-TREE.html#token-dest-attr)
