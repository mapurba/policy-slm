DirXMLScript DTD: token-replace-all element



# token-replace-all

**<token-replace-all>** replaces in the
result of the enclosed tokens each substring that
...
Note that pattern options CASE\_INSENSITIVE, DOTALL,
...
|| [**Tree**](DTD-TREE.html#token-replace-all)
