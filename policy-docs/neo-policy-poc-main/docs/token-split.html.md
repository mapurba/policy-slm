DirXMLScript DTD: token-split element



# token-split

**<token-split>**splits the result of the
enclosed tokens into a node-set consisting of text
nodes based on the pattern specified by
*delimiter*. If *csv* is true, then [CSV](http://en.wikipedia.org/wiki/Comma-separated_values) quoting rules will be honored during the
...
> | **delimiter** | **CDATA**   regular expression that matches the delimiter character(s)  *supports variable expansion* | #REQUIRED |
...
|| [**Tree**](DTD-TREE.html#token-split)
