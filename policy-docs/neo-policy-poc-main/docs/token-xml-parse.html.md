DirXMLScript DTD: token-xml-parse element



# token-xml-parse

**<token-xml-parse>** parses the result of
the enclosed tokens as XML and returns the
resulting document node in a node-set. If the
result of the enclosed tokens is not well-formed
XML or cannot be parsed for any reason, an empty
nodeset is returned.
...
|| [**Tree**](DTD-TREE.html#token-xml-parse)
