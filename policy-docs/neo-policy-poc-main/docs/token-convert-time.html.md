DirXMLScript DTD: token-convert-time element
...
See [Date/Time
Parameters](policy.html#datetime) for information on specifying formats,
...
> | **offset** | **CDATA**   the time offset (a negative number indicates subtract time interval)   *supports variable expansion* | #IMPLIED |
> | **offset-unit** | second   |  minute   |  hour   |  day   |  week   |  month   |  year   the units of the time offset   *supports variable expansion* | day |
...
|| [**Tree**](DTD-TREE.html#token-convert-time)
