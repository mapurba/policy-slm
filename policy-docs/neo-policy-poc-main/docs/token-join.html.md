DirXMLScript DTD: token-join element



# token-join

**<token-join>**joins the the values of
the nodes in the node-set result of the enclosed
tokens, separating the values by the characters
specified by *delimiter*. If *csv* is
true, then [CSV](http://en.wikipedia.org/wiki/Comma-separated_values) quoting rules will be applied to
the values.
...
> | --- | --- | --- |
> | **csv** | true   |  false   apply CSV quoting to values | false |
> | **delimiter** | **CDATA**   the string use to delimit the joined values  *supports variable expansion* | #IMPLIED |
