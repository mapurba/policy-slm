DirXMLScript DTD: token-parse-dn element
...
1. Typed Name Boolean Flag: '0' means names are NOT
   typed, '1' means names are typed
2. Unicode No-Map Character Boolean Flag: '0' means
...
> | --- | --- | --- |
> | **dest-dn-delims** | **CDATA**   specifies the custom DN delimiter set when dest *-dn-format*="custom" | #IMPLIED |
> | **dest-dn-format** | src-dn   |  dest-dn   |  dot   |  qualified-dot   |  slash   |  qualified-slash   |  ldap   |  custom   the format used to output the parsed DN | dest-dn |
...
> | **src-dn-delims** | **CDATA**   specifies the custom DN delimiter set when *src-dn-format*="custom" | #IMPLIED |
...
|| [**Tree**](DTD-TREE.html#token-parse-dn)
