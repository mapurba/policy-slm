DirXMLScript DTD: token-base64-encode element



# token-base64-encode

**<token-base64-encode>** converts
the result of the enclosed tokens to bytes using
the character set specified by *charset,* and
then base64 encodes the bytes.
...
> | --- | --- | --- |
> | **charset** | **CDATA**   charset used to convert the string to bytes  *if not specified, the encoding specified by the system property file.encoding is used  supports variable expansion* | #IMPLIED |
...
|| [**Tree**](DTD-TREE.html#token-base64-encode)
